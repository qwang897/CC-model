...
void gillespie(int kmax,double input[30]){
	//int 	Np_ini=(int)input[19],Nm_ini=(int)input[20],Np_max=(int)input[21],Nm_max=(int)input[22];
	...
	double calculate_s0(int Np,int Nm,int motor[2],double s0,int bind[100],double input[30],double s[100]){
		...
		//printf("%d\t%d\t%.2f\t%.2f\t%.2f\t",np,nm,s[1],s[2],s0);
		return s0;
	}
	...
	//printf("%.3f\t%.3f\t%.3f\t%.3f\n",n_tau_av_aF[1],n_tau_av_aF[2],n_tau_av_aF[3],n_tau_av_aF[4]);	
}
	...
	int i,i1;
	//int Np_max=(int)input[21],Nm_max=(int)input[22];
	for(i=0;i<=Np_max;i++)
