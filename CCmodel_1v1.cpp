...
int main(){
	...
	//again
		...
		if(np==0 && nm==0){
			...
			//if(k>snap0) break;
			// if(num>snap0 && choose_bidirection!=1)    break;
			// if(k_p>snap0 && k_m>snap0 && choose_bidirection==1)		break;
			if(num>snap0)		break;
		}

	if(choose_bidirection==1 && bid_sum==1)
		for(k=1;k<=snap0;k++)		{x_p[k]=x_p_sum[k];x_m[k]=x_m_sum[k];}


	s_max_sum=0;
	for(k=1;k<snap0;k++)		s_max_sum+=s_max_p[k];
	s_max_average_p[Np][Nm]=s_max_sum/snap0;
	s_max_sum=0;
	for(k=1;k<snap0;k++)		s_max_sum+=pow(s_max_p[k]-s_max_average_p[Np][Nm],2);
	s_max_e_p[Np][Nm]=sqrt(s_max_sum/(snap0*(snap0-1)));
	s_max_sum=0;
	for(k=1;k<snap0;k++)		s_max_sum+=s_max_m[k];
	s_max_average_m[Np][Nm]=s_max_sum/snap0;
	s_max_sum=0;
	for(k=1;k<snap0;k++)		s_max_sum+=pow(s_max_m[k]-s_max_average_m[Np][Nm],2);
	s_max_e_m[Np][Nm]=sqrt(s_max_sum/(snap0*(snap0-1)));
	...
	sum1=0;sum2=0;
	for(k=1;k<=snap0;k++){		sum1+=x_p[k];sum2+=x_m[k];}
	xp[Np][Nm]=sum1/snap0;xm[Np][Nm]=sum2/snap0;
	sum1=0;
	for(k=1;k<=snap0;k++)		sum1+=t_sum[k];
	T_sum[Np][Nm]=sum1/snap0;
